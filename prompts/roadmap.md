...
- Use `namespace TauCeti` by default. For dot-notation APIs whose first explicit argument has an
  ...
  Classic `import Mathlib...` syntax is simplest.
...
After opening a PR, end with a concise summary: the target and target roadmap you chose, the designated milestone it serves, why it was the most effective current step towards that milestone, the file(s) added and line count, the PR number/URL, the rubrics you read, and what your own review of the diff found and changed.
