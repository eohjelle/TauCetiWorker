...
## Choose a target
- **Choose the track.** `__ONLY__` is the designated roadmap; if it is `any`, first choose a non-skipped top-level area. Read its `README.md` under `__ROADMAP_DIR__/` in full. It is definitive; `Suggested.lean` is optional, non-exhaustive guidance.
- **Follow only explicit blockers.** `<target-roadmap>` starts as the designated roadmap. Switch it only when the current README assigns another canonical roadmap a missing prerequisite that blocks an unmet milestone. Read that roadmap's README and take its next missing target; repeat only along explicit blocking edges. Never roam to related areas or re-derive work the consumer says to consume. Cite the consumer milestone and upstream target in the PR body.
