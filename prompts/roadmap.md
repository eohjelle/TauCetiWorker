...
- Code goes under `TauCeti/`. Just create your new module there. Place it with the closest existing topic family, following the structure Mathlib and the surrounding tree already use: if a subdirectory for the topic exists, put the file there (`Foo/Bar.lean`); if the topic exists only as a small flat family of `Foo*.lean` files, follow the flat convention. But if your file would join an already-large flat family (several siblings share the CamelCase prefix; in practice four or more, matching the structure-nudge comment CI posts), you must RESTRUCTURE AS YOU ADD, in this same PR: `git mv` each sibling `FooBar.lean` to `Foo/Bar.lean` (keep any anchor `Foo.lean` exactly where it is; never invent `Basic.lean`; change only imports and module headers, zero declaration renames), place your new file in the new subdirectory, and list the old->new module table in the PR body. Never start a second layout beside a flat family. Exception: first scan open PRs for imports of the old module names (`gh pr diff` each); if one depends on them, use the flat convention this time and note the family on the structure tracking issue (https://github.com/TauCetiProject/TauCeti/issues/987) so the next PR after that conflict clears does the restructure. Do NOT edit the root `TauCeti.lean`: it is intentionally empty, and the lakefile's glob (`TauCeti.*`) builds and axiom-audits every module under `TauCeti/` without it being listed — hand-edits to the root only cause needless conflicts. Do NOT touch `Scripts/`, `.github/`, the lakefile (`lakefile.toml`/`lakefile.lean`), or the Lake pins (`lake-manifest.json`/`lean-toolchain`) — the lakefile is human-owned, and forward Mathlib/toolchain bumps are a separate dedicated flow; keep this PR to `TauCeti/`.
