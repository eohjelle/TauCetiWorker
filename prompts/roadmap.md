...
## Choose a target
- **Work ONLY within the `__FOCUS__` roadmap.** Read its plan under `__ROADMAP_DIR__/__FOCUS__/` (provided read-only) — especially `README.md` and `Targets.lean` — and pick a target (or a clean prerequisite a target there needs) from THAT area only. Do not pick targets from other roadmap areas. (If `__FOCUS__` is literally `any`, then any area under `__ROADMAP_DIR__/` is fair game.) Cite the exact target — e.g. `TauCetiRoadmap/__FOCUS__/README.md` plus the specific item — in the PR body.
- **Avoid duplicating open work.** List the PRs already in flight and read their titles and descriptions: `gh pr list --repo FormalFrontier/TauCeti --state open --limit 100 --json number,title,headRefName,body`. Also skim recently MERGED PRs (`--state merged`) so you build on, rather than repeat, what already landed. Do NOT pick a target an open or merged PR already covers or substantially overlaps (the same definition, the same roadmap item, or a near-identical API). Within `__FOCUS__`, prefer the lowest-hanging target not yet taken; if every easy one is in flight, pick a genuine prerequisite none of them supply. When in doubt that your idea is distinct, choose something else.
...
- **Claim it:**
  ```
  claim.sh acquire "author/__FOCUS__/<slug>"
  ```
...
- **Record it in the PR body** (required — the PR will be rejected without it): include the exact line
  ```
  <!--tauceti-target:v1 {"focus":"__FOCUS__","id":"<slug>"}-->
  ```
...
- Code goes under `TauCeti/`. You MAY — and for a new top-level module you SHOULD — also edit the root `TauCeti.lean` to add `import TauCeti.Your.New.Module`, so your API is reachable from the `TauCeti` root (the `api-design` and `placement` rubrics expect new modules to be reachable from the root, not only picked up by the build glob). Keep the import list alphabetically ordered. Do NOT touch `Scripts/`, `.github/`, `lakefile`, or `lake-manifest.json`.
