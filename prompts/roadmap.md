...
- Code goes under `TauCeti/`. Just create your new module there. Place it with the closest existing topic family, as Mathlib and the surrounding tree do: an existing topic subdirectory → put the file there (`Foo/Bar.lean`); a small flat `Foo*.lean` family → stay flat; a large one (four or more siblings share the prefix; the CI structure nudge flags this) → relocate FIRST, as a preliminary PR that becomes this round's PR: `git mv` each sibling `FooBar.lean` to `Foo/Bar.lean` (anchor `Foo.lean` stays put; no invented `Basic.lean`; only imports/module headers change, no declaration renames), with the old->new module table in the body; your roadmap target then lands on the restructured tree in a later round (keep your claim). Never bundle the relocation with new mathematics, and never start a second layout beside a flat family. One exception: if `gh pr diff` shows an open PR importing the old module names, skip the relocation, add your file flat, and note the family on https://github.com/TauCetiProject/TauCeti/issues/987. Do NOT edit the root `TauCeti.lean`: it is intentionally empty, and the lakefile's glob (`TauCeti.*`) builds and axiom-audits every module under `TauCeti/` without it being listed — hand-edits to the root only cause needless conflicts. Do NOT touch `Scripts/`, `.github/`, the lakefile (`lakefile.toml`/`lakefile.lean`), or the Lake pins (`lake-manifest.json`/`lean-toolchain`) — the lakefile is human-owned, and forward Mathlib/toolchain bumps are a separate dedicated flow; keep this PR to `TauCeti/`.
