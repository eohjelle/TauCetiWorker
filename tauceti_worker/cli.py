...
of work, chosen in priority order: rebase → bump → progress → fix-ci → fix → roadmap → review.
...
the cascade (priority order; a round does the first that applies):
  ...
  roadmap   open a new roadmap PR while fewer than 8 are open in the selected scope
  review    review an open PR (or when roadmap authoring is at capacity)
